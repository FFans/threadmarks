...
beforeEach(() => {
  ...
  jest.spyOn(app, 'handleErrorOnce').mockImplementation((...args: any[]) => {
...
test('late-loaded page replaces just the stream subtree when filtered mode is active', () => {
  class PageView {
    view() {
  ...
  addThreadmarkOnlyMode();
  flarum.reg.add('core', 'forum/components/DiscussionPage', PageView);
  state.set('threadmarksOnly', true);
  const filtered = { count: () => 0 };
  state.set('threadmarkStream', filtered);
  const view = new PageView().view();
  const children = view.children as any[];
  expect(children[0].children[0].children).toBe('Controls');
...
test.each([
  ...
  flarum.reg.add('core', 'forum/components/PostStream', WindowView);
  const children = new WindowView().view().children as any[];
...
test('jump flash waits for scrolling and ignores an obsolete jump when another target wins', async () => {
  ...
  flarum.reg.add('core', 'forum/components/PostStream', StreamView);
  const stream = new StreamView();
  const result = (stream.scrollToItem as any)($('<div data-index="1">'), true, false, false);
  stream.stream.threadmarkFlashIndex = 2;
  pending.resolve();
  await result;
  expect(stream.flashItem).not.toHaveBeenCalled();
  stream.stream.threadmarkFlashIndex = 1;
  await (stream.scrollToItem as any)($('<div data-index="1">'), true, false, false);
