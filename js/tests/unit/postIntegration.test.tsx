...
import addThreadmarkPostBlock from '../../src/forum/extenders/addThreadmarkPostBlock';
...
test('normal stream puts both marks before the intact post component and preserves unrelated items', () => {
  const body = m('article', 'Post content');
  jest.spyOn(PostStream.prototype, 'view').mockImplementation(() => m('div', [
    m('div', { className: 'PostStream-item', 'data-id': '11' }, body),
    m('aside', 'Other extension'),
  ]) as any);
  addThreadmarkPostBlock();
  const tree = PostStream.prototype.view.call({} as any) as any;
  const item = tree.children[0];
  expect(item.children[1]).toBe(body);
  expect(item.children[0].children.map((child: any) => child.tag)).toEqual([ThreadmarkBlock, ThreadmarkBlock]);
  expect(tree.children[1].tag).toBe('aside');
  data.discussion.pushData({ relationships: { threadmarks: [], personalThreadmarks: [] } });
  const empty = PostStream.prototype.view.call({} as any) as any;
  expect(empty.children[0].children[1]).toBe(body);
});


test('public editability comes from the serialized permission even without an included discussion', () => {
  data.publicMark.pushData({ relationships: { discussion: { data: null } }, attributes: { canManage: true } });
  expect(data.publicMark.canEdit()).toBe(true);
  data.publicMark.pushData({ attributes: { canManage: false } });
  expect(data.publicMark.canEdit()).toBe(false);
});
