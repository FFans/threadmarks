...
class ThreadmarkSchemaTest extends TestCase
{
    ...
    public function test_post_deletion_preserves_both_scopes_and_owner_deletion_only_removes_private_marks(bool $stringify): void
    {
        $this->db->getPdo()->setAttribute(PDO::ATTR_STRINGIFY_FETCHES, $stringify);
        $this->insert('personal', 1);
        $this->insert('personal', 2);
        $this->insert('discussion');
        ...
        foreach (['personal', 'discussion'] as $scope) {
            ...
            $this->assertSame(1, (int) $row->original_post_id);
            $this->assertSame(8, (int) $row->original_post_number);
        }
        $this->db->table('users')->where('id', 1)->delete();
        $this->assertSame([2], $this->db->table('ffans_personal_threadmarks')->pluck('owner_id')->map(fn ($id) => (int) $id)->all());
    ...
    public function fetchModes(): array
    {
        return ['native values' => [false], 'string values' => [true]];
    }
