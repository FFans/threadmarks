...
class PersonalMarksApiTest extends ForumTestCase
{
    public function test_independent_owners_crud_and_uniqueness_across_types(): void
    {
        ...
        $updated = $this->document($this->updateMark('personal-threadmarks', $mine['id'], 2, ['note' => null, 'position' => 2147483647], 2))['data'];
        $this->assertNull($updated['attributes']['note']);
        $this->assertSame(2147483647, $updated['attributes']['position']);
        $this->document($this->updateMark('personal-threadmarks', $mine['id'], 2, ['position' => 2147483648]), 422);
        $this->assertSame(2147483647, PersonalThreadmark::findOrFail($mine['id'])->position);
    ...
    public function test_private_notes_are_absent_from_every_other_viewer_and_direct_endpoint(): void
    {
        ...
        foreach ([0 => null, 1 => null, 2 => $mine['id'], 3 => $other['id'], 4 => null] as $actor => $id) {
            foreach (['/api/discussions/1', '/api/discussions/1?include=personalThreadmarks.type,threadmarks.type', '/api/posts/11?include=personalThreadmark.type'] as $path) {
    ...
    public function test_rejects_first_event_missing_posts_and_invalid_fields_without_persisting(): void
    {
        ...
        foreach ([['note' => str_repeat('字', 101)], ['position' => -1], ['position' => 2147483648], ['position' => 4294967295], ['position' => 4294967296], ['position' => 1.5]] as $attributes) {
