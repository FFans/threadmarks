...
class GdprAvailabilityTest extends ForumTestCase
{
    ...
    public function test_missing_gdpr_package_does_not_load_gdpr_classes_or_break_threadmarks(): void
    {
        ...
        // Core resolves Font Awesome CSS directly through the configured vendor path.
        (new \Symfony\Component\Filesystem\Filesystem())->mirror(
            $vendor.'/fortawesome/font-awesome/css',
            $shadow.'/vendor/fortawesome/font-awesome/css'
        );
        ...
        $previousVendor = getenv('FLARUM_TEST_VENDOR_PATH');
        spl_autoload_register($blockGdpr, true, true);
        putenv('FLARUM_TEST_VENDOR_PATH='.$shadow.'/vendor');
        try {
            $container = $this->app()->getContainer();
            ...
            putenv($previousVendor === false ? 'FLARUM_TEST_VENDOR_PATH' : 'FLARUM_TEST_VENDOR_PATH='.$previousVendor);
        }
    }
}
